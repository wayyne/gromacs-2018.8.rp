...
{   
    ...
    int c_gpuNumClusterPerCell = 8; //from the nbnxn_internal.h file

    //report #i-super-clusters in list, and number of atoms per super-cluster, total (i-super-cluster)  atoms in list
    printf("nsci: %d, na_sc: %d, nsci*na_sc: %d\n",
      nbl->nsci,nbl->na_sc,nbl->nsci*nbl->na_sc);

    //report #i-clusters in list, total number of i-clusters, # atoms per i-cluster, total (i-cluster) atoms in list 
    printf("nci: %d, nci_tot: %d, na_ci: %d, nci*na_ci: %d\n",
      nbl->nci,nbl->nci_tot,nbl->na_ci,nbl->nci*nbl->na_ci);

    //report the #j-clusters in the list, total number of 4*j-clusters, number of atoms per j-cluster, total (j-cluster) atoms in list
    printf("ncj: %d, ncj4: %d,  na_cj: %d, ncj*na_cj: %d\n",
      nbl->ncj,nbl->ncj4,nbl->na_cj,nbl->ncj*nbl->na_cj);
   
    //loop through the i-super cluster list 
    int i = 0;
    //for (int i = 0; i < nbl->nsci; i++)
    //{   
        fprintf(fp, "i-cluster = ci %4d  shift %2d  ncj4 %2d\n",
                  ...
                  gmx_walltime_accounting_t walltime_accounting)
{
    ...
    if (MASTER(cr))
    {
	    ...
	    //ROBIN_PROTOCOL NON-THREADED BLOCK
	    {
	
		if (ir->robin_protocol == 2 || ir->robin_protocol == 1){
                int delay,stride,pause,rounds,total;
		char *tok, *rest;

		//parse the injection pattern: delay,stride,pause,rounds
	     	rest = strdup(ir->rp_pattern);
		
		//delay
	      	tok = strtok_r(rest,",",&rest);
	      	delay = atoi(tok);

		//sanity check on the delay vs sample size
	      	if (delay < ir->rp_sample_sz)
	    	  delay = ir->rp_sample_sz;		
		...
		//NON-ACTIVE DUMP
	        if (ir->robin_protocol == 2 && (ir->rp_dump_vels || ir->rp_dump_rvels)){
                  //adjust stride if both forward and reverse velocities are requested
                  if (ir->rp_dump_vels && ir->rp_dump_rvels){
                    stride = (int)ceil(stride/2.0);
                    puts("RP Samples will represent both forward and reverse time evolution.");
                  }
                  else if (ir->rp_dump_rvels)
                    puts("RP Samples will represent reversed time evolution.");
                  else
                    puts("RP Samples will represent normal time evolution.");
      
		  if (ir->nsteps != -1){ 
	            printf("Nsteps set by RP to -1 (infinite) for nonactive dump run.\n");
	            ir->nsteps = -1;
		  }
                  else
	            printf("Nsteps set by user to -1 (infinite) for nonactive dump run.\n");

		  printf("This run will be terminated automatically by Robin Protocol.\n");
	        }
		//ACTIVE INFINITE RUN
		else if (ir->robin_protocol == 1 && ir->nsteps == -1){	
	          printf("User requested an infinite MLMD run (nsteps=-1)\n");

		  //total number of steps
		  ir->nsteps = delay+((stride+pause)*rounds);
	          printf("Nsteps adjusted by RP based on the requested injection pattern\n");
	          printf("RP updated nsteps=%d\n",ir->nsteps);
		}
    ...
    while (!bLastStep)
    {
        ...
        if (!bRerunMD || rerun_fr.bV || bForceUpdate)
        {
            ...
            print_nblist_sci_cj(stdout,fr->nbv->grp[0].nbl_lists.nbl[0]);
