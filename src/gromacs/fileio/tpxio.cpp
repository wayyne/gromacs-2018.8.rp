                        ...
                        int file_version, real *fudgeQQ)
{
    ...
    //Robin Protocol [ML-MD] Stuff
    gmx_fio_do_int(fio, ir->robin_protocol);
    gmx_fio_do_int(fio, ir->rp_fset);
    gmx_fio_do_int(fio, ir->rp_domain);
    gmx_fio_do_int(fio, ir->rp_sample_sz);
    gmx_fio_do_int(fio, ir->rp_alternative_stride);
    gmx_fio_do_int(fio, ir->rp_alternative_pause);
    gmx_fio_do_int(fio, ir->rp_reporter);
    gmx_fio_do_int(fio, ir->rp_threshold_type);
    gmx_fio_do_real(fio, ir->rp_threshold);
    gmx_fio_do_int(fio, ir->rp_dilation_factor);
    gmx_fio_do_gmx_bool(fio, ir->rp_alternate_patterns);
    gmx_fio_do_gmx_bool(fio, ir->rp_dump_vels);
    gmx_fio_do_gmx_bool(fio, ir->rp_dump_rvels);
    gmx_fio_do_gmx_bool(fio, ir->rp_balanced_diet);

    if ( ir->robin_protocol > 0 && bRead){

         char buf1[STRLEN];
         char buf2[STRLEN];

         gmx_fio_do_string(fio, buf1);
         ir->rp_pattern = gmx_strdup(buf1);

         gmx_fio_do_string(fio, buf2);
         ir->rp_groups = gmx_strdup(buf2);

         //parse the groups string to determine how many models we expect
         if (ir->rp_groups != nullptr){
           int Ngr = 0;
           for (int i=0;i<strlen(ir->rp_groups);i++)
             if (ir->rp_groups[i] == '[')
               Ngr += 1;

           //if we detected groups...then...
           if (Ngr > 0){
             //allocate space for each model path expected
             printf("RP: %d groups encountered...\n",Ngr);
             ir->rp_models = (char **)malloc(sizeof*(ir->rp_models)*Ngr);
             for (int i=0;i<Ngr;i++){
               snew(ir->rp_models[i],STRLEN);
               gmx_fio_do_string(fio, buf1);
               ir->rp_models[i] = gmx_strdup(buf1);
               ...
               if (strcmp(buf1,"none")){
                ...
                ir->rp_models[i] = NULL;
                printf("RP: model %d = %s (Robin Protocol is DISABLED for group %d)\n",i+1,ir->rp_models[i],i+1);
               }
      ...
      if (ir->robin_protocol > 0){
        ...
        printf("\nRobin protocol enabled."
               " \"So...I'm going to be in the Justice League someday?\" (Dick Grayson)\n\n");

        //validate the rp_pattern
        gmx_fio_do_string(fio, ir->rp_pattern);

        for (int j=0;j<strlen(ir->rp_pattern);j++)
          if (!isdigit(ir->rp_pattern[j]) && ir->rp_pattern[j] != ','){
            if (ir->rp_pattern[j] == ';' && j == strlen(ir->rp_pattern)-1)
              break;
            printf("Invalide RP pattern provided.\n");
            exit(1);
          }

        //validate model paths
        gmx_fio_do_string(fio, ir->rp_groups);
        ...
        if (ir->rp_groups != nullptr){
          ...
          if (Ngr > 0){
            //ir->rp_models = (char **)malloc(sizeof*(ir->rp_models)*Ngr);
            //snew(ir->rp_models,Ngr);
            for (int i=0;i<Ngr;i++){
              gmx_fio_do_string(fio, ir->rp_models[i]);
              if (!strcmp("none",ir->rp_models[i])){
                printf("RP: Model%d: none (Robin Protocol will be disabled for group %d)\n",i+1,i+1);
                Nmo++;
              } 
              else if (stat(ir->rp_models[i], &buffer) == 0){
                printf("RP: Model%d: %s\n", i+1,ir->rp_models[i]);
                Nmo++;
              }
            }
        ...
        if (Nmo != Ngr && ir->robin_protocol != 2){
          printf("RP_WARNING: Unable to validate Grayson models provided for Robin Protocol run. Goodbye.\n");
          exit(1);
        }
      }
    }
