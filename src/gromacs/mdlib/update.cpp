                            ...
                            const matrix box)
{
    ...
    for (int a = start; a < nrend; a += GMX_SIMD_REAL_WIDTH)
    {
        ...
        //ROBIN_PROTOCOLTHREADED BLOCK
        {
          if (rp_injection_circuit(f,v,x,a,nrend))
              simdLoadRvecs(v, a, &v0, &v1, &v2);
        }
                         ...
                         const matrix                M)
{
            ...
            if (haveSingleTempScaleValue)
            {
                ...
                if (!md->havePartiallyFrozenAtoms)
                {
                    updateMDLeapfrogSimpleSimd
                        (start, nrend, dt, md->invmass, tcstat, x, xprime, v, f,box);
                }
                else
#endif
                {
    ...
    //ROBIN_PROTOCOL THREADED BLOCK 
    {
      //historical record construction
      if (rp)
        if (step >= rp->pattern[DELAY] - rp->histr_shape[STEPS])
          for (int n = start; n < nrend; n++)
            rp_round_robin(n,xprime,v,step);
    }
}
...
static void do_update_vv_pos(int start, int nrend, real dt,
                             ...
                             gmx_bool bExtended, real veta, gmx_int64_t step)
{
    int    gf = 0;
    int    n, d;
    real   g, mr1, mr2;
    ...
    for (n = start; n < nrend; n++)
    {
        ...
        for (d = 0; d < DIM; d++)
        {
            if ((ptype[n] != eptVSite) && (ptype[n] != eptShell) && !nFreeze[gf][d])
            {
  //      	 if (rp_injection_circuit(v,x,n,nrend))
//			;
...
#pragma omp parallel for num_threads(nth) schedule(static)
    ...
    if (!(bFirstHalf)) /* in the first half of vv, no shift. */
    {
        ...
        {
            puts("md->cFREEZE != nullptr && constr != nullptr == TRUE");
                   ...
                   gmx_constr_t                   constr)
{
    ...
    //ROBIN_PROTOCOL NON-THREADED BLOCK
    {
      ...
      if (inputrec->robin_protocol > 0 && rp == NULL){
        init_robin_protocol(state,inputrec,md);

        //handle special case
        if (step == rp->pattern[DELAY] && step == 0){
          const rvec *x_rvec  = as_rvec_array(state->x.data());
          rvec *v_rvec  = as_rvec_array(state->v.data());

          for (int n = 0; n < state->natoms; n++)
            rp_round_robin(n,x_rvec,v_rvec,step);
        }
      }

      //non-active dumping
      if (rp)
        if ((rp->dump_rvels || rp->dump_vels) && !rp->active)
          rp_nonactive_dump(step);
    }
...
#pragma omp parallel for num_threads(nth) schedule(static)
    ...
    {
      //housekeeping
      if ((step == inputrec->init_step+inputrec->nsteps) && rp)
        free_robin_protocol(rp);
    }
}
