...
struct t_inputrec
{
    ...
    /* robin protocol 3.0 */
    int                      robin_protocol;    //toggle RP between: disabled, inactive, active
    int                      rp_fset;           //toggle between feature sets
    int                      rp_domain;         //-1 = whole system, otherwise specifies particular molecule to target
    int                      rp_sample_sz;      //set the sample size
    int                      rp_alternative_stride;
    int                      rp_alternative_pause;
    int                      rp_reporter;       //atom index for RP print statements
    char                    *rp_pattern;        //delay, stride, pause, rounds;
    char                   **rp_models;
    char                    *rp_groups;
    int                      rp_threshold_type; //none, magnitude, xyz
    float                    rp_threshold;
    int                      rp_dilation_factor;
    gmx_bool                 rp_alternate_patterns;
    gmx_bool                 rp_dump_vels;
    gmx_bool                 rp_dump_rvels;
    gmx_bool                 rp_balanced_diet;
