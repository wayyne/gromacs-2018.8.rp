            ...
            WriteMdpHeader writeMdpHeader, warninp_t wi)
{
    ...
    /* robin protocol (MLMD) stuff */
    snew(ir->rp_pattern,        STRLEN);
    snew(ir->rp_groups,         STRLEN);
    CCTYPE("Robin Protocol [ML-MD] stuff");
    EETYPE("robin_protocol", ir->robin_protocol, rp_modes);
    EETYPE("rp_fset", ir->rp_fset, rp_fsets);
    ITYPE ("rp_domain", ir->rp_domain, -1);
    ITYPE ("rp_sample_sz", ir->rp_sample_sz, -1);
    ITYPE ("rp_alternative_stride", ir->rp_alternative_stride, -1);
    ITYPE ("rp_alternative_pause", ir->rp_alternative_pause, -1);
    ITYPE ("rp_reporter", ir->rp_reporter, -1);
    EETYPE("rp_threshold_type", ir->rp_threshold_type, rp_threshold_types);
    RTYPE ("rp_threshold", ir->rp_threshold, -1.0);
    ITYPE ("rp_dilation_factor", ir->rp_dilation_factor, -1);
    EETYPE("rp_alternate_patterns", ir->rp_alternate_patterns, yesno_names);
    EETYPE("rp_dump_vels", ir->rp_dump_vels, yesno_names);
    EETYPE("rp_dump_rvels", ir->rp_dump_rvels, yesno_names);
    EETYPE("rp_balanced_diet", ir->rp_balanced_diet, yesno_names);
    STYPE ("rp_pattern", ir->rp_pattern, nullptr);
    STYPE ("rp_groups", ir->rp_groups, nullptr);

    //we need to parse the groups string to determine how many models we expect
    if (ir->rp_groups != nullptr){
      ...
      if (Ngr > 0){
        ...
        for (int i=0;i<Ngr;i++){
          ...
          char buf[100] = {0};
          sprintf(buf,"rp_model%d",i+1);
          //printf("Looking for %s\n",buf);
          STYPE (buf, ir->rp_models[i], nullptr);
          //printf("\tFound: %s\n",ir->rp_models[i]);
        }
      } 
    }
